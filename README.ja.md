...
> [!WARNING]
> **アーカイブ・非推奨。** 本プロジェクトはメンテナンスを終了し、`sentinels` マーケットプレイスから撤去しました。4つのテンプレート(architecture, api, domain, setup)はいずれもコードからエージェントが都度再構築できる構造説明であり、継続的な鮮度フックは維持コストに見合いません。コードから復元できない決定の理由は、代わりにADRで残してください。
